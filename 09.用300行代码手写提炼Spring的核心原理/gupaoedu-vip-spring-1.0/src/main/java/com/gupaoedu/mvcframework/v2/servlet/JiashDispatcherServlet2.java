 ...
 * @Description:
 */
public class JiashDispatcherServlet2 extends HttpServlet {
    ...
    //初始化，为DI做准备 doInstance()方法就是工厂模式的具体实现   IOC 容器就是注册时单例的具体案例
    ...
    public class Handler {
        ...
        private void putParamIndexMapping(Method method) {
            ...
            for (int i=0;i<ans.length;i++){
                Annotation[] as = ans[i];
                for (Annotation a : as) {
                    if(a instanceof JiashRequestParam){
                        String paramName = ((JiashRequestParam)a).value();
